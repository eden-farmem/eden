 ...
 * @block: do not yield the core until the fault is resolved
 */
	...
	int evprio, bool block)
{
	...
	fault->is_blocking = block ? 1 : 0;
#ifdef BLOCKING_HINTS
	/* set all hinted faults as blocking */
	fault->is_blocking = 1;
#endif
    fault->thread = myth;
    log_debug("%s - fault posted", FSTR(fault));
